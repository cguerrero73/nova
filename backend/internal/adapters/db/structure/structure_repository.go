...
func (r *PgStructureRepository) FindByChild(ctx context.Context, childCode, childOrg string) ([]*structure.Structure, error) {
	...
	rows, err := infraDB.GetQueryEngine(ctx, r.pool).Query(ctx, query, childCode, childOrg)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *PgStructureRepository) Update(ctx context.Context, s *structure.Structure) error {
	...
	_, err := infraDB.GetQueryEngine(ctx, r.pool).Exec(ctx, query, s.ID, s.Cost, s.Meter, s.UpdatedAt)
	return err
}
...
func (r *PgStructureRepository) DeleteByParentChild(ctx context.Context, parentCode, parentOrg, childCode, childOrg string) error {
	...
	_, err := infraDB.GetQueryEngine(ctx, r.pool).Exec(ctx, query, parentCode, parentOrg, childCode, childOrg)
	return err
}
