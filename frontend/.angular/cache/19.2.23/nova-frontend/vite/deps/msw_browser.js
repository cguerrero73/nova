...
// node_modules/.pnpm/msw@2.12.14_@types+node@22.19.15_typescript@5.8.3/node_modules/msw/lib/core/utils/internal/mergeRight.mjs
...
// node_modules/.pnpm/msw@2.12.14_@types+node@22.19.15_typescript@5.8.3/node_modules/msw/lib/core/utils/toResponseInit.mjs
...
// node_modules/.pnpm/msw@2.12.14_@types+node@22.19.15_typescript@5.8.3/node_modules/msw/lib/core/utils/internal/isHandlerKind.mjs
...
function emitAsync(emitter, eventName, ...data) {
  return __async(this, null, function* () {
    ...
    for (const listener of listeners) yield listener.apply(emitter, data);
  });
}
...
var WebSocketOverride = class extends EventTarget {
  ...
  constructor(url, protocols) {
    ...
    queueMicrotask(() => __async(this, null, function* () {
      if (yield this[kPassthroughPromise]) return;
...
// node_modules/.pnpm/msw@2.12.14_@types+node@22.19.15_typescript@5.8.3/node_modules/msw/lib/core/ws/webSocketInterceptor.mjs
var webSocketInterceptor = new WebSocketInterceptor();

// node_modules/.pnpm/msw@2.12.14_@types+node@22.19.15_typescript@5.8.3/node_modules/msw/lib/core/ws/handleWebSocketEvent.mjs
function handleWebSocketEvent(options) {
  webSocketInterceptor.on("connection", (connection) => __async(null, null, function* () {
    const handlers = options.getHandlers().filter(isHandlerKind("EventHandler"));
    if (handlers.length > 0) {
      options?.onMockedConnection(connection);
      yield Promise.all(handlers.map((handler) => {
    ...
    yield onUnhandledRequest(request, options.getUnhandledRequestStrategy()).catch((error2) => {
...
// node_modules/.pnpm/msw@2.12.14_@types+node@22.19.15_typescript@5.8.3/node_modules/msw/lib/browser/index.mjs
...
function until(callback) {
  return __async(this, null, function* () {
    try {
      return [null, yield callback().catch((error2) => {
        throw error2;
      })];
    } catch (error2) {
      return [error2, null];
...
var getWorkerInstance = (_0, ..._1) => __async(null, [_0, ..._1], function* (url, options = {}, findWorker) {
  ...
  const [registrationError, registrationResult] = yield until(() => __async(null, null, function* () {
...
function enableMocking(context, options) {
  ...
  context.workerChannel.once("MOCKING_ENABLED", (event) => __async(null, null, function* () {
    context.isMockingEnabled = true;
    const worker = yield context.workerPromise;
    printStartMessage({
      quiet: options.quiet,
...
var RequestController = class RequestController2 {
  ...
  */
  passthrough() {
    return __async(this, null, function* () {
      invariant2.as(InterceptorError, this.readyState === RequestController2.PENDING, 'Failed to passthrough the "%s %s" request: the request has already been handled', this.request.method, this.request.url);
  ...
  */
  respondWith(response) {
    invariant2.as(InterceptorError, this.readyState === RequestController2.PENDING, 'Failed to respond to the "%s %s" request with "%d %s": the request has already been handled (%d)', this.request.method, this.request.url, response.status, response.statusText || "OK", this.readyState);
  ...
  */
  errorWith(reason) {
    invariant2.as(InterceptorError, this.readyState === RequestController2.PENDING, 'Failed to error the "%s %s" request with "%s": the request has already been handled (%d)', this.request.method, this.request.url, reason?.toString(), this.readyState);
...
var BatchInterceptor = class BatchInterceptor2 extends Interceptor2 {
  constructor(options) {
...
var createStartHandler = (context) => {
  return function start(options, customOptions) {
    const startWorkerInstance = () => __async(null, null, function* () {
...
Please consider using a custom "serviceWorker.url" option to point to the actual worker script location, or a custom "findWorker" option to resolve the Service Worker registration manually. More details: https://mswjs.io/docs/api/setup-worker/start`, options.serviceWorker.url, location.host);
    ...
    const workerRegistration = startWorkerInstance().then((registration) => __async(null, null, function* () {
  ...
  emitAsPromise(event) {
    return __async(this, null, function* () {
      ...
      for (const listener of this.#matchListeners(event.type)) {
        ...
        if (proxiedEvent.event[kImmediatePropagationStopped]) break;
        const returnValue = yield Promise.resolve(this.#callListener(proxiedEvent.event, listener));
...
var WorkerChannel = class extends Emitter2 {
  constructor(options) {
    ...
    navigator.serviceWorker.addEventListener("message", (event) => __async(this, null, function* () {
      const worker = yield this.options.worker;
...
var until2 = (promise) => __async(null, null, function* () {
  try {
...
function handleRequest2(options) {
  return __async(this, null, function* () {
    const handleResponse = (response) => __async(null, null, function* () {
    ...
    const handleResponseError = (error2) => __async(null, null, function* () {
      if (error2 instanceof InterceptorError) throw result.error;
      if (isNodeLikeError(error2)) {
        yield options.controller.errorWith(error2);
        return true;
      }
      if (error2 instanceof Response) return yield handleResponse(error2);
      return false;
    });
    ...
    const result = yield until2(() => __async(null, null, function* () {
      const requestListenersPromise = emitAsync2(options.emitter, "request", {
        requestId: options.requestId,
        request: options.request,
        controller: options.controller
      });
      yield Promise.race([requestAbortPromise, requestListenersPromise, options.controller.handled]);
    }));
    ...
    if (result.error) {
      if (yield handleResponseError(result.error)) return;
    ...
    if (options.controller.readyState === RequestController.PENDING) return yield options.controller.passthrough();
      ...
      globalThis.fetch = (input, init) => __async(this, null, function* () {
        ...
        const controller = new RequestController(request, {
          passthrough: () => __async(this, null, function* () {
            ...
            } = yield until2(() => pureFetch(request));
...
function isDomParserSupportedType(type) {
  return ["application/xhtml+xml", "application/xml", "image/svg+xml", "text/html", "text/xml"].some((supportedType) => {
...
function getBodyByteLength(input) {
  return __async(this, null, function* () {
    ...
    return (yield input.arrayBuffer()).byteLength;
  });
}
...
var XMLHttpRequestController = class {
  ...
  */
  toFetchApiRequest(body) {
    ...
    define(fetchRequest, "headers", createProxy(fetchRequest.headers, {
      methodCall: ([methodName, args], invoke) => {
        switch (methodName) {
          case "append":
          case "set": {
...
function createFallbackRequestListener(context, options) {
  ...
  interceptor.on("request", (_0) => __async(null, [_0], function* ({
    request,
    requestId,
    controller
  }) {
...
var SetupWorkerApi = class extends SetupApi {
  ...
  constructor(...handlers) {
    super(...handlers);
    invariant2(!isNodeProcess(), devUtils.formatMessage("Failed to execute `setupWorker` in a non-browser environment. Consider using `setupServer` for Node.js environment instead."));
